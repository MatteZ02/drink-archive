...
module.exports = class database {
    ...
    select (options) {
      ...
      if (options.where) {
        ...
        if (options.username && options.password) {
          ...
          this.connection.query(`select ${options.fields} from ${options.table} where ${options.where}`, [options.username, options.password], (error, results, fields) => {
            resolve(results);
          });
        });
         } else
        return new Promise(resolve => {
        this.connection.query(`select ${options.fields} from ${options.table} where ${options.where}`, (error, results, fields) => {
          resolve(results);
        });
      });
        ...
        return new Promise(resolve => {
          this.connection.query(`select ${options.fields} from ${options.table}`, (error, results, fields) => {
            resolve(results);
          });
        });
      }
    ...
    insert (options) {
      ...
      if (typeof options.values !== "string") throw new Error(`values must be of type "string" type received was ${typeof options.values}.`);
        return new Promise(resolve => {
          console.log(`insert into ${options.table} (${options.fields}) values (${options.values})`);
          this.connection.query(`insert into ${options.table} (${options.fields}) values (${options.values})`, (error, results, fields) => {
            resolve(results);
          });
        });
    }
